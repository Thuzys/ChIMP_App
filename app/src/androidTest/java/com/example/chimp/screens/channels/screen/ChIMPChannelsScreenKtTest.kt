...
class ChIMPChannelsScreenKtTest {
    ...
    @get:Rule
    val fakeUserInfoRepositoryRule = FakeUserInfoRepositoryRule()

    @get:Rule
    val fakeChannelRepositoryRule = FakeChannelRepositoryRule()

    private val connectivity = flowOf(CONNECTED)
    ...
    fun scrolling_state_view_in_Channel_screen() {
        ...
        val vm = ChannelsViewModel(
            service = service,
            userInfoRepository = fakeUserInfoRepositoryRule.repo,
            channelRepository = fakeChannelRepositoryRule.repo,
