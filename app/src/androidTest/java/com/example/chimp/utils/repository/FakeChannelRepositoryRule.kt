...
class FakeChannelRepositoryRule : TestRule {
    ...
    val repo = object : ChannelRepository {
        override val channelInfo = MutableStateFlow<ChannelBasicInfo?>(null)

        override suspend fun updateChannelInfo(channel: ChannelBasicInfo) = channelInfo.emit(channel)
    ...
    companion object {
        val emptyChannel = ChannelBasicInfo(
            cId = 1u,
            name = ChannelName("Channel1", "Channel1"),
            owner = UserInfo(1u, "Owner1")
        )

        val channelWithMessages = ChannelBasicInfo(
            cId = 2u,
